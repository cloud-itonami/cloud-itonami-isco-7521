(ns woodtreatcoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [woodtreatcoord.store :as store]
            [woodtreatcoord.advisor :as advisor]
            [woodtreatcoord.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-treater! st {:treater-id "treater-1" :name "Aki Sato"})
    (store/register-facility! st {:facility-id "F-1" :name "Kobo Treatment Facility" :max-supply-cost 2000})
    st))
...
(def ^:private req {:treater-id "treater-1"})
...
(deftest hard-on-unregistered-treater
  (let [st (fresh-store)
        v (governor/check {:treater-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-treater (:rule %)) (:violations v)))))
...
(deftest hard-on-scope-excluded-op-finalize-treatment-decision
  (testing "finalizing a treatment-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-treatment-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-finalize-treatment-operation
  (testing "finalizing the treatment operation is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-treatment-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-authorize-treatment-run
  (testing "authorizing a treatment run is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-treatment-run) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-proceed-with-treatment-run
  (testing "proceeding with a treatment run is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :proceed-with-treatment-run) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-op-declare-batch-safe-for-handling
  (testing "declaring a treated batch safe for handling is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :declare-batch-safe-for-handling) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-declare-treated-batch-safe-for-handling
  (testing "declaring a treated batch safe for handling (alternate op name) is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :declare-treated-batch-safe-for-handling) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-treatment-run
  (testing "defense-in-depth: a rationale that itself attempts to proceed with the treatment run is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we proceed with the treatment run now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-rationale-chemical-safety-clearance
  (testing "defense-in-depth: a rationale that itself attempts to declare the treated batch safe for handling is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :log-work-record)
                                           :rationale "we can declare the treated batch safe for handling now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-rationale-finalize-treatment-operation
  (testing "defense-in-depth: a rationale that itself attempts to finalize the treatment operation is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :coordinate-supply-order :cost 100)
                                           :rationale "recommend we finalize the treatment operation today")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'wood'/'preservative'/'chemical'/'treatment', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:treater-id "treater-1" :op o :facility-id "F-1"
                        :stake :low :task "routine wood treatment task"
