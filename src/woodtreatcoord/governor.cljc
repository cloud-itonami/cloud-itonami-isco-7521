(ns woodtreatcoord.governor
  "WoodTreatCoordGovernor — the independent safety/scope layer gating
  every treatment-facility scheduling/logistics proposal an advisor may
  make for a wood-treatment crew. The governor never dispatches
  hardware itself, never performs wood treatment work itself, and
  never finalizes a treatment-execution decision (e.g. deciding to
  proceed with a specific wood-treatment run) or a chemical-safety-
  clearance decision (e.g. declaring a treated batch safe for handling
  ...
  this actor coordinates TREATMENT-FACILITY SCHEDULING/LOGISTICS ONLY —
  it never performs wood treatment work itself). Modeled closely on
  cloud-itonami-isco-7514's preservecoord.governor.

  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    1. treater provenance   — the crew member must be independently
                                verified/registered before any action.
    2. facility provenance  — the treatment-facility site must be
                                independently verified/registered
                                before any action.
    3. no-actuation           — proposal :effect must be :propose (the
                                governor never dispatches hardware and
                                never performs wood treatment work
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                treatment-execution decision (e.g.
                                deciding to proceed with a specific
                                wood-treatment run), or a chemical-
                                safety-clearance decision (e.g.
                                declaring a treated batch safe for
                                ...
                                such as \"finalize the treatment
                                operation\" / \"declare the treated
                                batch safe for handling\" / \"override
                                the shop safety officer's judgment\",
                                never as bare nouns like \"wood\",
                                \"preservative\", \"chemical\" or
                                \"treatment\", so the check can never
                                self-trip on the advisor's own routine
                                rationale text, e.g. \"logged work
                                record for treater …\" or \"scheduled
                                crew operation for treatment task …\"
  ...
  This actor coordinates treatment-facility scheduling/logistics ONLY
  — it never performs wood treatment work itself, and it never makes a
  chemical-safety-clearance decision itself; those decisions always
...
(def ^:private scope-excluded-ops
  #{:finalize-treatment-decision :finalize-treatment-operation
    :authorize-treatment-run
...
;; ("wood", "preservative", "chemical", "treatment", "creosote", "CCA",
;; "safety", "shop", "officer") — so this can never match inside the
;; mock advisor's own default rationale text (which legitimately
;; contains those bare nouns, e.g. "treatment task" / "shop safety
;; officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the treatment run" "proceed with the wood treatment run"
   "authorize the treatment run" "authorize the wood treatment run"
   "finalize the treatment decision" "finalize the treatment operation"
   "declare the batch safe for handling" "declare the treated batch safe for handling"
   "declare the batch safe for shipment" "declare the treated batch safe for shipment"
   "clear the batch for shipment" "clear the treated batch for handling"
...
(defn- hard-violations [proposal treater-record facility-record]
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      (nil? treater-record)
      (conj {:rule :no-treater
             :detail "未登録 treater への提案は不可（treater record は独立して検証・登録済みでなければならない）"})
      ...
      (conj {:rule :no-actuation
             :detail "effect は :propose のみ許可（governor は薬剤処理作業を直接実行しない）"})
...
(defn check
  ...
  `store` implementing `woodtreatcoord.store/Store`. Pure — never
  mutates the store, never dispatches a treatment-facility operation."
  [request _context proposal store]
  (let [treater-record (store/treater store (:treater-id request))
        facility-record (some->> (:facility-id proposal) (store/facility store))
        hard (hard-violations proposal treater-record facility-record)
